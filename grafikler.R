...
library(scales)
ormansal <- read.csv(
  "ormansal_atiklar.csv",
  sep       = ";",
  fileEncoding = "CP1254",   # Türkçe karakter desteği
  stringsAsFactors = FALSE,
  check.names = FALSE
)
names(ormansal)
df <- ormansal %>%
  ...
  mutate(
    ...
    endustriyel_olmayan = as.numeric(
      str_replace_all(as.character(endustriyel_olmayan), ",", ".")
    )
  ) %>%
  arrange(desc(endustriyel_olmayan)) %>%
  slice_head(n = 10) %>%
  mutate(
    bolge_kisa = factor(bolge_kisa, levels = rev(bolge_kisa)),
    rank = row_number()
  )
fmt_tr <- function(x) {
...
renk_paleti <- c(
  "#b8d8f8", "#9ec4ee", "#8ab2e4", "#7aa2dc", "#6a93d4",
  "#5a84cc", "#4a75c4", "#3a65b8", "#2a52a0", "#1a3a6b"
)
p <- ggplot(df, aes(x = bolge_kisa, y = endustriyel_olmayan)) +
  ...
  labs(
    title    = "Endustriyel Olmayan Ormansal Atiga Sahip 10 İl Bolge Mudurlugu",
    subtitle = "Olcek: Ster / Yil",
    x        = NULL,
    y        = "Atik Miktari (Ster/Yil)",
    caption  = "Kaynak: Ormansal Atiklar Veri Seti"
  ) +
  
  theme_minimal(base_size = 12) +
  theme(
    plot.background  = element_rect(fill = "#f4f7f4", color = NA),
    panel.background = element_rect(fill = "#f4f7f4", color = NA),
    plot.title    = element_text(face = "bold", size = 16, color = "#1a3a6b",
                                 margin = margin(b = 4)),
    plot.subtitle = element_text(size = 10, color = "#555f55",
                                 margin = margin(b = 14)),
    plot.caption  = element_text(size = 8, color = "#999999", hjust = 0),
    plot.margin   = margin(20, 40, 15, 20),
    panel.grid.major.y = element_blank(),
    panel.grid.minor   = element_blank(),
    panel.grid.major.x = element_line(color = "#dce8dc", linetype = "dashed"),
    axis.text.y  = element_text(size = 11, color = "#1a3a6b", face = "bold"),
    axis.text.x  = element_text(size = 9,  color = "#777777"),
    axis.title.x = element_text(size = 9,  color = "#777777", margin = margin(t = 8))
  )
print(p)
